...
std::pair<open_spiel::Action, float> _LogABSpiel(std::unique_ptr<State>& state)
{
...
std::pair<abalone_core::core_Action, float> _LogAB(std::unique_ptr<State>& state)
{
  auto game = state->GetGame();
  Player player = state->CurrentPlayer();
  auto start_time = std::chrono::high_resolution_clock::now();
  auto best_move = abalone_core::AlphaBeta(static_cast<abalone::AbaloneState*>(state.get())->core_state_, kSearchDepthAbalone, -1.f, 1.f);
  auto end_time = std::chrono::high_resolution_clock::now();
  std::chrono::duration<float, std::milli> elapsed = end_time - start_time;

  std::cout << std::endl << "(core_ab) Player " << player << " choosing action "
...
class AbaloneHeuristicEvaluator : public open_spiel::algorithms::Evaluator {
 public:
  ...
  std::vector<double> Evaluate(const State& state) override
  {
    ...
    auto score = abalone_core::Heuristic(abalone_state.core_state_, core_player) * 0.001;
...
{
  auto game = state->GetGame();
  auto evaluator = std::make_shared<open_spiel::algorithms::RandomRolloutEvaluator>(20, 42);
  ...
  //   root->children.begin(),
  //   root->children.end(),
  //   [](const auto& e1, const auto& e2){return (e1.total_reward/e1.explore_count)<(e2.total_reward/e2.explore_count);}
  // );
  auto best_child = root->BestChild();
  std::cout << "mcts"
    << " action: " << abalone_core::Move::ActionToMove(best_child.action).ToString()
...
void DatasetTest()
{
		...
		> scenarios =
	{
      ...
      std::string("classic"),
			116,
			std::string("1.a1b2 i5h5 2.a5b5 i6h6 3.b5c5 i7h7 4.b6b5 i8h8 5.b1c2 h9g8 6.a3b3 h7g7 7.b4c4 h4h5 8.a4b5a3 e6e7 9.a3a2 h6g6 10.b3c3 i9i8 11.a2b3 i8h8 12.b4b3 f8e7 13.d3d4 h7g6 14.b1b2 e4f5 15.b4c4 d7d8 16.d6c5 d8e8 17.b4b3 f8g9 18.b1b2 g9f8 19.b4b3 f5g6 20.c2d3 f8f7 21.b2c2 i8i7 22.b1a1b2 i7i8 23.a2b2a1 i8h8 24.b1b2 f5g6 25.b2c2 i8h7 26.d2c2 g5h6 27.f4e3 h6g5 28.d2c2 e6f6 29.a2b2 h6g6 30.d2c2 f8e7 31.a2b2 d6e7 32.d2c2 f5g6 33.a2b2 f8f7 34.d3d4 h7g6 35.d6c5 i8h7 36.b4c4 e6f6 37.e3e4 f4g4 38.d2e3 g4f4 39.e4e5 h6g6 40.e7d6 e9e8 41.b4c4 e6f6 42.d6c5 f4g4 43.b3c4 h6g6 44.b2b3 h7h6 45.d6c5 f7f6 46.c2d3 f4g5 47.d3e4 i7h7 48.d5e5 f7f6 49.a3b3 h7g7 50.b3c4 i5h4 51.g6g5 g3h4 52.f4g4 h6h5 53.g5f4 h4h5 54.c5d5 i8h8 55.g4f3 g5h6g6 56.b4c5 h5i5h6 57.d3e4 g8g7 58.g5f4 i6i7"),
			// 1 - 1 tie
    ...
    auto game = LoadGame(std::string("abalone(board=") + game_board + std::string(")"));
    auto state = game->NewInitialState();
		// state.reset(*);
		std::cout << state << std::endl;
		//auto eval = state.eval();
		//std::cout << "finished: " << std::get<0>(eval) << " eval: " << std::get<1>(eval) << std::endl;
		...
		for (i = 0; ; ++i)
		{
			...
			std::cout << "=> p0: " << std::get<0>(p1_move_flag) << " " << p1_move.ToString() << std::endl;
      ...
      if(i>=42)
      {
        ...
        // std::vector<std::pair<abalone_core::core_Action, float>> move_abalone;
        // abalone_core::AlphaBeta(static_cast<abalone::AbaloneState*>(state.get())->core_state_, kSearchDepthAbalone, -1.f, 1.f, &move_abalone);
        // SPIEL_CHECK_EQ(move_spiel.size(), move_abalone.size());
        // for(int i=0; i<move_spiel.size(); ++i)
        // {
        //   SPIEL_CHECK_EQ(move_spiel[i].first, move_abalone[i].first);
        //   SPIEL_CHECK_FLOAT_NEAR(move_spiel[i].second, move_abalone[i].second, 1e-3);
        // }
      ...
      state->ApplyAction(moveId1);
			...
			std::cout << "=> p1: " << std::get<0>(p2_move_flag) << " " << p2_move.ToString() << std::endl;
    ...
    auto core_state = static_cast<abalone::AbaloneState*>(state.get())->core_state_;
		game_result res = std::get<3>(scenario);
		auto board_balls_P0 = 14 - std::count(&core_state.board_[0][0], &core_state.board_[0][0] + sizeof(core_state.board_) / sizeof(core_state.board_[0][0]), abalone_core::CellState::Player1);
		auto board_balls_P1 = 14 - std::count(&core_state.board_[0][0], &core_state.board_[0][0] + sizeof(core_state.board_) / sizeof(core_state.board_[0][0]), abalone_core::CellState::Player0);
		auto result_balls_P0 = std::get<0>(res);
		auto result_balls_P1 = std::get<1>(res);
		std::cout << "board balls P0: " << board_balls_P0 << " expected balls P0: " << result_balls_P0 << std::endl;
		std::cout << "board balls P1: " << board_balls_P1 << " expected balls P1: " << result_balls_P1 << std::endl;
		std::cout << "board result: " << core_state.outcome_ << " expected game result: " << std::get<2>(res) << std::endl;
...
void StringAbaloneTests() {
  // SPIEL_CHECK_TRUE(false);
  for (auto i = 0; i < abalone_core::kNumCells*abalone_core::kNumActionsPerCell; ++i) {
    auto move = abalone_core::Move::ActionToMove(i);
    auto move_str = move.ToString();
    std::cout << move_str << std::endl;
    printf(move_str.c_str());
    // auto maybe_move = abalone_core::Move::from_string(move_str);
    // if(std::get<0>(maybe_move))
    // {
    //   auto move2 = std::get<1>(maybe_move);
    //   auto action = MoveToAction(move2);
